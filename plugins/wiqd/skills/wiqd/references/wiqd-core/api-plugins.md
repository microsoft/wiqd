...
After running `wiqd agent add action`, complete the following before delivering a customized, provision-ready action. For generation-only requests, leave the generated files intact and report the remaining checklist items instead of customizing or provisioning:
...
5. **Verify authentication and required inputs** using [Configure and Verify API Plugin Authentication](#configure-and-verify-api-plugin-authentication).
6. **Provision when requested**, after required configuration is complete, using `wiqd agent provision --env local` for the local environment or the environment requested by the user.

The `wiqd agent add action` command generates scaffolding, not a finished integration. Apply the customization requirements below when customization is in scope; do not claim the action is provision-ready while required configuration remains incomplete.
...
| ---------------- | ------------------------------------------------------------------------------------ |
| `--openapi-spec` | URL or local file path to the OpenAPI specification. wiqd auto-detects URL vs. file. |
| `--operations`   | Required comma-separated `METHOD /path` selectors to include.                         |
| `--folder`       | Agent project directory (default: `./`).                                             |
| `--json`         | Emit structured output.                                                              |

### Local File Paths

Local OpenAPI paths may be absolute or relative to `--folder`:

```bash
--openapi-spec ./openapi.json --folder /home/user/project
```
...
| "File not found" during `wiqd agent add action`  | Local path is not relative to `--folder`                   | Use an absolute path or a path relative to `--folder`                    |
...
## Configure and Verify API Plugin Authentication

When adding an API action or configuring authentication for an existing action:

1. **Use the authentication declared in the OpenAPI spec.** If the user
  requests authentication or supplies credentials but the selected operations
  have no applicable security scheme, explain that `wiqd agent add action`
  cannot configure authentication from credentials alone and clarify the
  intended setup. Do not add or change authentication without an explicit
  request. Otherwise, the absence of security requirements alone does not
  require provider research or a clarification question.

2. **Use the CLI to generate authentication configuration.** Use
  `wiqd agent add action` to add the API, then inspect the generated
  authentication configuration. Do not call `wiqd agent add auth` as a
  follow-up to adding an action or to fill missing credentials. Use
  `wiqd agent add auth` only when the user explicitly asks to configure
  authentication for an existing API specification with no authentication
  configured. Otherwise, report missing authentication configuration without
  invoking that command. Check the active command's `--help` for supported
  options; do not assume another backend's flags are available.

3. **Check credentials deferred for manual entry before provisioning.** During
  `wiqd agent add action`, the user may choose to enter credentials manually
  in the generated environment files instead of supplying them to the command.
  Inspect the inputs of the generated `oauth/register` or `apiKey/register`
  action and check whether the client ID, client secret, API key, or other
  user-supplied values required by that authentication mode are still missing
  for the requested environment. Tell the user the exact missing variable
  names and generated file locations to complete securely before provisioning
  can succeed. Do not treat registration IDs or other values produced by
  earlier lifecycle steps as credentials the user must fill in. Never request
  secrets in chat, print their values, or commit them.

4. **Respect the requested stopping point.** For generation-only requests,
  leave the project intact and report any remaining authentication setup.
  If provisioning was requested and required manual inputs are missing,
  report the blocker and wait for the user to set them securely before running
  provisioning. Successful generation or provisioning
  does not prove that OAuth sign-in or authenticated API calls work.

Consult [authentication.md](./authentication.md) only for provider-specific
OAuth details needed for the requested setup. This section's explicit-request,
secret-handling, and CLI-generation rules take precedence over conflicting
instructions in that guide. Do not invent scopes or replace CLI-generated
registration steps with its examples. Dynamic Client Registration is optional:
use it only when supported by the provider and chosen flow, and authorized by
the user; existing or manually registered clients do not require it.
...
- [Plugin Manifest Schema](schema.md) — see "Discovering the current published version" for how to find the current Learn page and versioned JSON Schema
