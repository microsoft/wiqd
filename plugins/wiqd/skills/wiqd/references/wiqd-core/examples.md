...
wiqd agent provision --json --env local
```
...
wiqd agent share --json --scope tenant --env dev
```
...
> **Version placeholder.** In the example manifests below, `v<x.y>` stands for the current published version — do not copy it literally. Use the version your scaffolder emitted (read it from the generated manifest's `$schema`/`version`), or discover the current version per [schema.md](schema.md).
...
A complete agent combining all features, on a manifest version of at least **v1.8** — the minimum version required for `EmailActions` and `MeetingActions` (see the version placeholder note above for the `$schema`/`version` fields) — note the new `EmailActions` and `MeetingActions` write-action capabilities, each paired with matching instructions and a conversation starter. In this example, `Email` is scoped to the `support@contoso.com` shared mailbox for reads, while `EmailActions` is unscoped and independent of `Email`: its write operations target the signed-in user's own mailbox, and `Email` scoping such as `shared_mailbox` or `folders` does not apply to it:
...
```json
{
  ...
  "instructions": "You are a professional customer support agent for Contoso.\n\nResponsibilities:\n1. Search the knowledge base for relevant documentation\n2. Look up repair tickets and their status\n3. Create new repair tickets when requested\n4. Search support emails for context\n5. Triage the signed-in user's own inbox — flag, move, and file messages, and draft supervised replies for the user to review\n6. Schedule follow-up meetings and propose times when a call is needed\n\nGuidelines:\n- Always be polite and professional\n- Cite sources when providing information\n- Ask clarifying questions when needed\n- Draft email replies and meeting invites for the user to review; never send without confirmation\n- Never share confidential information",
  "capabilities": [
    {
      "name": "OneDriveAndSharePoint",
      "items_by_url": [
  ...
  "conversation_starters": [
    ...
    },
    {
      ...
      "text": "Flag urgent messages in my mailbox and file the rest into folders"
    },
    {
      "title": "Schedule a Follow-up",
      "text": "Set up a follow-up meeting with the customer next week"
    }
  ],
  "disclaimer": {
