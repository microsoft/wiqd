...
> - MCP action → runtime dynamic discovery, or pinned `mcp_tool_description.tools[]` with full
>   `inputSchema`
...
| -------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------- |
| Interpretation | Literal — follows instructions step-by-step as written | Intent-first — interprets what instructions _intended_, not just what they said |
...
- **Ambiguous instructions** → GPT 5.2 will _replan and improvise_, which may produce unexpected behavior: reordered steps, merged tasks, tone drift, added/removed steps based on inferred context
...
Tells the model the _shape_ of the answer — tone, format, length, style — but gives it no strategy for _finding_ the answer.
...
Tells the model the _decision logic_ — WHEN to use which tool, in what order, and what to do when things fail. Does NOT duplicate tool descriptions or parameters already in the plugin metadata.
...
| C6  | **Hallucination invitation**       | "Include [specific data] in your response" without specifying where to find it                                         | Add: "Retrieve [data] from [capability]. If not found, do not include it." If the agent should ONLY respond from configured data sources, also set `behavior_overrides.special_instructions.discourage_model_knowledge: true` in the manifest (v1.4+) — this provides platform-level enforcement beyond instruction-level guidance. |
...
| `EmailActions`          | **Write** actions on the signed-in user's own mailbox (flag/move/file messages, draft supervised replies) — distinct from the read-only `Email` capability, and not scoped by `Email`'s `shared_mailbox`/`folders` | "triage my inbox", "flag/file emails", "draft a reply for me to review", "clean up my mailbox"                                                      |
| `MeetingActions`        | **Write** actions to create/propose meetings and calendar events              | "schedule a meeting", "set up a follow-up", "propose times", "send an invite"                                                                       |
...
| `EmailActions`                   | v1.8            | Write actions on the signed-in user's mailbox (flag, move, file, draft replies — independent of `Email` read scoping) |
| `MeetingActions`                 | v1.8            | Create/propose meetings and calendar write actions                   |
...
**Grounding control:**
- [If `behavior_overrides.special_instructions.discourage_model_knowledge` is set to `true`: "✅ Agent is configured to discourage model knowledge — it will prefer 'I don't know' over fabricated answers."]
- [If NOT set but instructions say "only answer from documents" or "do not use general knowledge": "⚠️ Instructions request grounded-only behavior but `discourage_model_knowledge` is not enabled in the manifest. Consider adding `behavior_overrides` for platform-level enforcement (v1.4+)."]
- [If NOT set and not implied by instructions: no comment needed]

**Version upgrade opportunities:**
- [If the current schema version is not the latest published version, list capabilities available in newer versions that could benefit this agent's intent. Confirm the current latest against the public schema source (see the ATK `schema.md` overview) rather than assuming a hardcoded number. Example: "You're on v1.4. Upgrading to v1.5 would unlock `Meetings` (calendar + transcripts), which aligns with your instruction's references to meeting prep and scheduling."]
- [If already on the latest published version: "✅ You're on the latest schema version — no upgrade needed."]
...
5. **No hallucination invitations** — every "include X" statement has a corresponding "retrieve X from Y" instruction. For agents that must only answer from configured data sources, verify `behavior_overrides.special_instructions.discourage_model_knowledge: true` is set in the manifest (v1.4+)
